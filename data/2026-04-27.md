...
TL;DR: This paper presents a hardware-software co-design framework that enables deterministic deployment of PyTorch-defined SNNs to event-driven FPGA execution using a single exported artifact, achieving 87.40% accuracy on MNIST with low latency and energy consumption.


<details>
  <summary>Details</summary>
Motivation: Low-cost FPGA platforms can broaden access to neuromorphic research, but existing SNN workflows are fragmented between hardware-first implementations (hard to integrate with PyTorch) and software-first frameworks (limited to simulation or GPU). A unified, semantics-preserving software-to-board path is needed.

Method: The authors design a semantics-preserving co-design framework where a single exported artifact (containing weights, thresholds, connectivity, and TTFS decoding metadata) is reused identically by both the software reference and the FPGA board runtime, enabling deterministic event-driven deployment.

Result: On a 10-class MNIST TTFS classifier at 80 MHz: 87.40% accuracy matching software reference on all 10,000 test images; 0.1375 μs/image programmable-logic latency; 31.6 nJ/image estimated dynamic energy; scope-aware comparisons with GPU/CPU baselines.

Conclusion: Low-cost event-driven FPGA hardware can provide a direct, reproducible, and semantics-preserving software-to-board path for software-defined SNN models, bridging the gap between PyTorch development and hardware execution.
...
TL;DR: GR-Evolve uses LLM agents to automatically adapt global routing source code to individual chip designs, achieving up to 8.72% wirelength reduction.


<details>
  <summary>Details</summary>
Motivation: Existing EDA tools rely on fixed heuristics and hyperparameter tuning, failing to adapt their internal algorithms to individual designs, which limits QoR and increases manual tuning effort.

Method: GR-Evolve is an LLM-driven code evolution framework that iteratively modifies global routing source code using QoR feedback, persistent contextual knowledge, and an integrated evaluation toolchain within OpenROAD.

Result: Up to 8.72% reduction in post-detailed-routing wirelength across seven benchmarks and three technology nodes compared to baseline routers.

Conclusion: LLM-driven EDA code evolution shows strong potential for design-adaptive global routing, moving beyond static heuristic-based tools.
...
TL;DR: This paper proposes using high-level design features from HLS to create helper assertions that guide model checking, achieving up to 6.05x speedup (2.23x average) for verifying HLS-generated RTL designs.


<details>
  <summary>Details</summary>
Motivation: HLS-generated RTL designs may contain functional bugs or security vulnerabilities, and formal verification (model checking) is reliable but often struggles due to the large size of generated RTL designs.

Method: The authors utilize high-level design features from the HLS flow to construct helper assertions that guide the model checker. They develop a proving mechanism that iteratively reuses proving information to select the most effective set of helpers.

Result: On HLS design benchmarks, the approach achieves up to 6.05x speedup and 2.23x average speedup compared to vanilla model checking.

Conclusion: Leveraging high-level design information through helper assertions can significantly accelerate formal verification of HLS-generated RTL designs.
...
TL;DR: HGQ-LUT accelerates LUT-aware neural network training by 100x+ on GPUs while achieving state-of-the-art hardware efficiency, and provides an end-to-end workflow with automatic quantization and resource optimization.


<details>
  <summary>Details</summary>
Motivation: Existing LUT-aware training (LAT) approaches are too slow, require manual tuning, and lack an integrated workflow, hindering practical deployment.

Method: Introduces LUT-Dense and LUT-Conv layers using regular tensor operations during training, combined with fine-grained heterogeneous quantization (including zero-bit pruning) and a LUT-aware resource surrogate for automatic accuracy-resource trade-off exploration.

Result: Achieves over 100x training speedup on modern GPUs compared to prior LAT methods, with state-of-the-art hardware efficiency on FPGAs.

Conclusion: HGQ-LUT makes LUT-based DNNs practical for real-world deployment, including at the CERN Large Hadron Collider, by enabling automatic, efficient, and end-to-end LUT-aware design.
...
TL;DR: This paper introduces a specialized matrix-matrix multiplication (mmul) kernel schedule for Coarse-Grained Reconfigurable Arrays (CGRAs) and a novel compilation methodology using polyhedral transformations to expose hidden mmul operations, achieving speedups up to 9.1x across various benchmarks.


<details>
  <summary>Details</summary>
Motivation: Existing compilation strategies for CGRAs produce suboptimal results when mapping computational kernels containing mmul, due to the multi-dimensional structure of mmul hiding its inherent parallelism and limiting runtime performance.

Method: The authors introduce a specialized, parametrizable mmul CGRA kernel schedule and a novel compilation methodology that uses polyhedral transformations (loop reordering and splitting) to analyze complex patterns, expose hidden mmul operations, substitute them with optimized assembly, and generate CGRA configurations combining pre-compiled and compiled parts.

Result: The experimental results demonstrate speedups up to 9.1x across different benchmarks containing hidden mmuls and CGRA instances of various sizes.

Conclusion: The proposed strategy maximizes resource utilization and runtime performance, even when mmul is not directly apparent in the source code, outperforming state-of-the-art compilation approaches.
...
TL;DR: This work analyzes microarchitectural inefficiencies in the open-source RVV vector processor Ara, identifies three key bottlenecks (memory-side, control-side, and operand-delivery), and proposes coordinated optimizations that achieve a 1.33x speedup and close 12.2% of the performance gap to the theoretical bound without increasing hardware resources.


<details>
  <summary>Details</summary>
Motivation: Modern RISC vector processors suffer from significant sustained-throughput loss due to microarchitectural inefficiencies, despite theoretical advantages from multi-lane parallelism and chaining. The gap between achieved and theoretical performance needs to be systematically identified and addressed.

Method: Established an ideal multi-lane chaining execution model as a reference, attributed bottlenecks along three critical paths (memory-side, control-side, operand-delivery), and proposed coordinated microarchitectural optimizations targeting these inefficiencies.

Result: Geometric-mean speedup of 1.33x; geometric-mean gap-closed ratio of 12.2%; scal (2.41x, 93.7%), axpy (1.60x, 88.9%), ger (1.52x, 78.3%), gemm (1.42x, 59.3%).

Conclusion: The proposed optimizations effectively recover sustained-throughput capability lost to microarchitectural inefficiencies without increasing hardware resources, moving workloads significantly closer to the theoretical performance bound.
...
### [7] [COMPASS: A Unified Decision-Intelligence System for Navigating Performance Trade-off in HPC](https://arxiv.org/abs/2604.22688)
*Ankur Lahiry,Banooqa Banday,Yugesh Bhattarai,Tanzima Z. Islam,Mohammad Zaeed*

Main category: cs.PF

TL;DR: COMPASS is a modular engine that uses operational traces to generate HPC configuration recommendations, cutting job turnaround time by 65.93% and node usage by 80.93% compared to state-of-the-art methods.


<details>
  <summary>Details</summary>
Motivation: Existing autotuners recommend good configurations but fail to identify minimal changes for near-miss configurations to meet performance objectives, and they often ignore domain-specific constraints.

Method: COMPASS formalizes configuration questions into query patterns, develops an interactive decision-making engine formulating queries as ML tasks, quantifies recommendation trustworthiness via evidence and uncertainty estimation, and provides guidance on which configurations to run next when confidence is low.

Result: Validated using analytical ground truth, reconstruction accuracy, reproduction of published findings, and real hardware tests. Achieves 65.93% reduction in average job turnaround time and 80.93% reduction in node usage. Up to 100x faster training and 80x faster inference, scaling to 1.3B samples and 126GB data.

Conclusion: COMPASS effectively bridges the gap in HPC configuration tuning by providing evidence-backed, trustworthy recommendations and outperforms state-of-the-art methods in both efficiency and scalability.
...
### [8] [Shard the Gradient, Scale the Model: Serverless Federated Aggregation via Gradient Partitioning](https://arxiv.org/abs/2604.22072)
*Amine Barrak*

Main category: cs.DC

TL;DR: GradsSharding partitions gradient tensors into shards for serverless FL aggregation, enabling arbitrarily large models by avoiding per-function memory limits, with cost savings and deployability beyond existing serverless FL approaches.


<details>
  <summary>Details</summary>
Motivation: Existing serverless FL architectures (lambda-FL, LIFL) cannot scale because each aggregator must hold the complete model gradient in memory, hitting per-function memory ceilings (e.g., 10 GB on AWS Lambda) for large models.

Method: GradsSharding partitions the gradient tensor into M shards, each independently averaged by a serverless function receiving contributions from all clients. Since FedAvg averaging is element-wise, this produces bit-identical results to tree-based methods while bounding per-function memory at O(|θ|/M).

Result: Evaluated on HPC and AWS Lambda from 43 MB to 5 GB models. Cost crossover at ~500 MB gradient size, 2.7x cost reduction at VGG-16 scale, and is the only architecture deployable beyond the serverless memory ceiling.

Conclusion: GradsSharding overcomes the fundamental memory scalability barrier in serverless FL aggregation, enabling large-model FL on memory-constrained serverless platforms with cost efficiency.
...
### [9] [FlashSpread: IO-Aware GPU Simulation of Non-Markovian Epidemic Dynamics via Kernel Fusion](https://arxiv.org/abs/2604.22092)
*Heman Shakeri,Behnaz Moradi-Jamei,Aram Vajdi,Ehsan Ardjmand*

Main category: cs.DC

TL;DR: FlashSpread is a GPU framework that fuses non-Markovian epidemic simulation into a single Triton kernel, achieving up to 8.09 Giga-NUPS on A100 — a 217x speedup over CPU — and scaling to 100M nodes, with ~6-7% bias versus exact Gillespie reference.


<details>
  <summary>Details</summary>
Motivation: Non-Markovian (renewal) epidemic simulation on large contact networks is crucial for realistic forecasting with age-dependent distributions, but the dense per-step hazard updates make standard sparse CPU event-queue methods inefficient, motivating a GPU approach.

Method: FlashSpread consolidates the entire renewal pipeline (CSR traversal, erfcx-based hazard, Bernoulli tau-leaping, state transition, write-back) into a single fused Triton kernel with block-scalar skips for CUDA Graph capture and degree-aware CSR dispatch (thread/warp/edge-merge) optimized for scale-free graphs.

Result: On A100: 8.09 Giga-NUPS at N=10^6 (217x over CPU tau-leaping); 4.5x improvement on Barabási-Albert graphs via merge dispatch; scales to N=10^8 on single A100; mixed-precision gives 3x L2-reachable scale and 1.15x throughput; ~6-7% bias vs exact Gillespie reference.

Conclusion: FlashSpread enables massive-scale non-Markovian epidemic simulation on GPUs with practical accuracy, offering orders-of-magnitude speedup and scalability well beyond CPU methods.
...
### [10] [Guess-Verify-Refine: Data-Aware Top-K for Sparse-Attention Decoding on Blackwell via Temporal Correlation](https://arxiv.org/abs/2604.22312)
...
TL;DR: GVR is a data-aware exact Top-K algorithm for sparse-attention decoding on NVIDIA Blackwell that exploits temporal correlation across consecutive decode steps, achieving 1.88x speedup over production kernels and up to 7.52% end-to-end TPOT improvement at 100K context.


<details>
  <summary>Details</summary>
Motivation: Exact Top-K selection in sparse-attention decoders is a latency bottleneck for long-context LLM serving, even with optimized indexer and attention kernels.

Method: GVR uses a Guess-Verify-Refine approach: predicts Top-K from previous step, computes pre-indexed statistics, narrows threshold via secant-style counting, verifies with ballot-free collector, and selects exactly in shared memory. It exploits Toeplitz/RoPE structure of DeepSeek Sparse Attention scores.

Result: 1.88x single-operator speedup (up to 2.42x per layer) over production radix-select kernel; 7.52% end-to-end TPOT improvement at 100K context on DeepSeek-V3.2 with TensorRT-LLM on Blackwell.

Conclusion: GVR effectively reduces Top-K latency in sparse-attention decoding and may generalize to other decoders where Top-K exhibits temporal stability.
...
### [11] [GICC: A High-Performance Runtime for GPU-Initiated Communication and Coordination in Modern HPC Systems](https://arxiv.org/abs/2604.22126)
*Baodi Shan,Mauricio Araya-Polo,Barbara Chapman*

Main category: cs.DC

TL;DR: GICC enables GPU kernels to directly trigger NIC-level coordination without host involvement, reducing latency by up to 229x on Slingshot and 1.95x on InfiniBand, while improving scaling efficiency.


<details>
  <summary>Details</summary>
Motivation: Distributed GPU applications lack kernel-level, cross-node coordination support; existing runtimes rely on host-driven progress and lack bounded mechanisms for recycling NIC work across repeated GPU-triggered operations, causing unnecessary overhead.

Method: GICC decouples coordination semantics from data movement and introduces asynchronous resource reclamation where the NIC signals completion to both GPU and host, allowing a lightweight host thread to recycle NIC resources concurrently with GPU execution.

Result: On Slingshot: up to 229x lower per-coordination latency, 25% better weak scaling. On InfiniBand: up to 1.95x lower put latency than NVSHMEM. On 64 AMD MI250X GCDs: 42% parallel efficiency vs. MPI's 35.4%.

Conclusion: GICC provides a practical framework for GPU-driven distributed coordination that sustains performance under finite NIC state, eliminating host overhead and unnecessary synchronization across both Slingshot and InfiniBand interconnects.
...
TL;DR: This paper proposes an LP-guided approximation algorithm for coflow scheduling in multi-core OCS networks under asynchronous reconfiguration, achieving 8K and 8K+1 approximation ratios for zero and arbitrary release times, with applicability to EPS networks as well.


<details>
  <summary>Details</summary>
Motivation: Existing coflow scheduling research mainly focuses on EPS multi-core networks, with limited or poor-performance results for OCS networks, despite OCS being increasingly deployed in DCNs for high bandwidth and energy efficiency.

Method: The authors develop an algorithm integrating LP-guided global coflow ordering, inter-core flow allocation, and intra-core circuit scheduling, handling cross-core coupling and port exclusivity/reconfiguration overhead constraints under the not-all-stop (asynchronous) reconfiguration model.

Result: Approximation ratios of 8K and 8K+1 for zero and arbitrary release times in OCS networks (K cores), and 4H and 4H+1 for EPS networks (H cores).

Conclusion: The proposed framework effectively tackles coflow scheduling in multi-core OCS networks with strong theoretical guarantees, and is also extendable to EPS networks.
...
### [13] [Accelerating Intra-Node GPU-to-GPU Communication Through Multi-Path Transfers with CUDA Graphs](https://arxiv.org/abs/2604.22228)
*Amirhossein Sojoodi,Yiltan Hassan Temucin,Amirreza Baratisedeh,Hamed Sharifian,Ahmad Afsahi*

Main category: cs.DC

TL;DR: This paper integrates CUDA Graphs into UCX for multi-path GPU communication (NVLink, PCIe), achieving up to 2.95x bandwidth improvement over single-path methods.


<details>
  <summary>Details</summary>
Motivation: To reduce communication overhead and improve execution efficiency in intra-node multi-path GPU communication for MPI-based HPC applications.

Method: Integrates CUDA Graphs into the UCX framework, concurrently leveraging multiple communication paths (NVLink and PCIe through the host) and optimizing workflows using CUDA Graph.

Result: Achieves up to 2.95x bandwidth improvement over single-path UCX (UCT::CUDA-IPC) in GPU-to-GPU OMB bandwidth tests at message sizes up to 512MB on a four-GPU node.

Conclusion: The proposed CUDA Graph-based multi-path approach significantly enhances intra-node GPU communication performance, and is the first to seamlessly integrate CUDA Graphs into UCX.
...
### [14] [A comprehensive evaluation of spatial co-execution on GPUs using MPS and MIG technologies](https://arxiv.org/abs/2604.22430)
*Jorge Villarrubia,Luis Costero,Francisco D. Igual,Katzalin Olcoz*

Main category: cs.DC

TL;DR: This paper evaluates NVIDIA's MPS and MIG spatial sharing technologies, finding MPS offers flexibility but suffers memory contention (30% degradation), while MIG provides isolation with higher overhead and rigidity. MPS improves performance by 30% and reduces energy by 20% in favorable scenarios.


<details>
  <summary>Details</summary>
Motivation: GPU computational resources are increasingly underutilized, motivating the need for spatial sharing methods that allow multiple applications to use GPUs simultaneously.

Method: Comprehensive evaluation of NVIDIA's Multi-Process Service (MPS) and Multi-Instance GPU (MIG) technologies for spatial GPU sharing.

Result: MPS improves performance by 30% and reduces energy by 20% under favorable conditions, but degrades by 30% under memory contention. MIG provides consistent improvements via hardware isolation but with higher overhead and rigidity.

Conclusion: There is a crucial trade-off between MPS's flexibility and MIG's isolation; co-execution strategies should be tailored according to job profiles.
...
### [15] [LaissezCloud: Continuous Resource Renegotiation for the Public Cloud](https://arxiv.org/abs/2604.22509)
*Tejas Harith,Antoine Kaufmann*

Main category: cs.DC

TL;DR: LaissezCloud is a cloud resource management platform that enables continuous re-negotiation of running allocations through online bidding, improving performance under contention by 8-23% versus traditional on-demand and spot instances.


<details>
  <summary>Details</summary>
Motivation: Current cloud allocation interfaces (on-demand and spot instances) are too rigid to handle time-varying tenant objectives and operator constraints in heterogeneous, oversubscribed clusters, especially without exposing internal application or infrastructure state.

Method: LaissezCloud keeps allocations continuously contestable during execution; tenants and operators update bids online, and running tenants retain resources only as long as their bid exceeds competing demand. Pricing serves as a narrow waist and incentive-alignment mechanism without exposing internal telemetry.

Result: Across diverse accelerator workloads, LaissezCloud reduces performance degradation under contention by 8-23% versus on-demand and spot baselines, and scales to clusters of at least 10,000 nodes.

Conclusion: Continuous re-negotiation of allocations via online bidding outperforms rigid service classes, aligning incentives between mutually untrusted tenants and operators while preserving privacy of internal state.
